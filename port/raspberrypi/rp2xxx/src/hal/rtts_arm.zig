    ...
    return struct {
        ...
        pub fn start_cores() noreturn {
            ...
            if (RTTS.core_count > 1) {
                // if (interrupt.can_set_handler()) {
                //     if (compatibility.chip == .RP2040) {
                //         _ = interrupt.set_handler(.SIO_IRQ_PROC0, .{ .c = fifo_ISR });
                //         _ = interrupt.set_handler(.SIO_IRQ_PROC1, .{ .c = fifo_ISR });
                //     } else {
                //         _ = interrupt.set_handler(.SIO_IRQ_BELL, .{ .c = doorbell_ISR });
                //     }
        ...
        fn run_first_task() noreturn {
            ...
            if (RTTS.core_count > 1) {
                ...
                if (compatibility.chip == .RP2040) {
                    if (core_id() == 0) {
                        interrupt.enable(.SIO_IRQ_PROC0);
                    } else {
                        interrupt.enable(.SIO_IRQ_PROC1);
                    }
                } else {
                    _ = multicore.doorbell.read_and_clear();
                    interrupt.enable(.SIO_IRQ_BELL);
                }
