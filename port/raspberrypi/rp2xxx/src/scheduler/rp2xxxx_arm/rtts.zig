...
pub fn Configure( comptime RTTS : type ) type
{
  return struct
  {
    ...
    pub fn startCores() noreturn
    {
      ...
      null_task_stack[null_task_stack_len - 2] = @intFromPtr( &nullTaskLoop );
    ...
    pub fn significantEvent() void
    {
      ...
      else
      {
        ...
        sendFifoCommand( .dispatch, null, null );
      }
    }
    ...
    fn svc_ISR() callconv( .c ) void
    {
      ...
      std.log.debug( "{s}SVC: {}", .{debugCore(), @intFromEnum( svc_id )} );
    ...
    pub fn pendsv_ISR() callconv( .naked ) noreturn
    {
      ...
      // regisgters and the stack pointer.
      ...
      asm volatile (
        ...
        \\    bx    lr
        :
        : [find_next] "s" (&RTTS._findNextTaskSP)
      );
    }
    ...
    fn runFirstTask() noreturn
    {
      ...
      std.log.debug( "{s}Entered runFirstTask", .{debugCore()} );

      var target_sp : [*]usize  = &null_task_stack;
      var target_pc : usize     = @intFromPtr( &nullTaskLoop );

      {
        ...
        for (RTTS.task_list) |*a_task|
        {
          ...
          {
            RTTS.current_task[coreID()] = a_task;
            a_task.state = .running;
            target_sp = a_task.stack_pointer;
            target_pc = @intFromPtr( a_task.loop );

            std.log.debug( "{s}   Starting task {s}", .{ debugCore(), a_task.name } );
            break;
          }
        }
      }
    ...
    fn core1_init() noreturn
    {
      std.log.debug( "{s}Init", .{debugCore()} );
      runFirstTask();
    }
    ...
    fn sendFifoCommand( in_command : FIFOCommand,
                        in_task    : ?*RTTS.TaskItem,
                        in_param   : ?u32 ) void
    {
    ...
    fn nullTaskLoop() callconv(.naked) noreturn
    {
      // while (true)
      // {
