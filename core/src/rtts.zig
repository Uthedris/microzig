...
pub fn scheduler(comptime tasks: []const Task, comptime config: Config) type {
    ...
    const EventFlagsTag = @Int(.unsigned, config.event_flags_count);

    comptime var event_flags_tag : [config.event_flags_count][]const u8 = undefined;
    comptime var event_flags_num : [config.event_flags_count]EventFlagsTag = undefined;

    comptime for (0..config.event_flags_count) |i| 
    { 
        event_flags_tag[i] = std.fmt.comptimePrint( "event_flag_{d}", .{i} );
        event_flags_num[i] = 1 << i; 
    };

    const EventFlagType = @Enum( EventFlagsTag,
                                 .exhaustive,
                                 &event_flags_tag,
                                 &event_flags_num );
    ...
    return struct {
        ...
        const EventFlags = @Int(.unsigned, config.event_flags_count);
        ...
        pub var task_stacks: blk: {
            ...
            break :blk [size]usize;
        } = undefined;

        /// The task tag type.  An enum of task name with values of task index.
        pub const TaskTag = blk: {
            // Construct the TaskTag enum
            var task_tag_tag : [tasks.len][]const u8 = undefined;
            var task_tag_num : [tasks.len]u8 = undefined;

            for (0..tasks.len) |i| {
                task_tag_tag[i] = tasks[i].name;
                task_tag_num[i] = i;
            }

            break :blk @Enum( u8, .exhaustive, &task_tag_tag, &task_tag_num );
        };
